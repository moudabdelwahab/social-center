...
(async function () {
  ...
  const TYPES = {
    trigger: { label: 'محفّز', color: '#c9964a', icon: 'zap', items: ['بداية الحملة', 'موعد محدد', 'اكتمال عملية', 'فشل عملية', 'تغيّر حالة حساب'] },
    condition: { label: 'شرط', color: '#5f9aa8', icon: 'branch', items: ['هل الحساب متصل؟', 'هل العملية نجحت؟', 'هل الحملة نشطة؟', 'هل تجاوزنا حد الطلبات؟'] },
    action: { label: 'إجراء', color: '#4f9d7e', icon: 'send', items: ['نشر محتوى', 'جدولة منشور', 'مزامنة الحسابات', 'جمع الإحصائيات', 'إرسال إشعار', 'إعادة المحاولة'] }
  };
  let A = { nodes: [], edges: [], seq: 0 };
  try { const saved = await DB.automation.load(); if (saved?.graph) A = saved.graph; } catch (e) { console.error(e); }
  let dirty = false;
  const markDirty = () => { dirty = true; document.getElementById('tb-save')?.classList.add('btn-primary'); };

  const pal = document.getElementById('palette');
  pal.innerHTML = Object.entries(TYPES).map(([t, cfg]) => `
    <h4>${cfg.label === 'محفّز' ? 'المحفزات' : cfg.label === 'شرط' ? 'الشروط' : 'الإجراءات'}</h4>
  ...
  const wirePath = (x1, y1, x2, y2) => { const dx = Math.max(50, Math.abs(x2 - x1) / 2); return `M${x1},${y1} C${x1 + dx},${y1} ${x2 - dx},${y2} ${x2},${y2}`; };

  function nodeEl(n) {
    const cfg = TYPES[n.type];
    const el = document.createElement('div');
    el.className = `bnode t-${n.type}`;
    el.style.left = n.x + 'px'; el.style.top = n.y + 'px'; el.dataset.nid = n.id;
    el.innerHTML = `
    ...
    el.querySelector('.bn-del').onclick = (e) => {
      ...
      markDirty(); renderAll(); toast('حُذفت العقدة', { type: 'info', duration: 1500 });
    };
    const out = el.querySelector('.port.out'), inp = el.querySelector('.port.in');
    out.addEventListener('pointerdown', (e) => {
      e.stopPropagation(); wiring = n.id;
      tempPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      tempPath.setAttribute('class', 'temp'); wires.appendChild(tempPath);
      const mv = (ev) => {
        const cr = canvas.getBoundingClientRect();
        tempPath.setAttribute('d', wirePath(n.x + 210, n.y + 40, ev.clientX - cr.left + canvas.scrollLeft, ev.clientY - cr.top + canvas.scrollTop));
      };
      const up = () => { removeEventListener('pointermove', mv); removeEventListener('pointerup', up); tempPath?.remove(); tempPath = null; wiring = null; };
    ...
    if (inp) inp.addEventListener('pointerup', (e) => {
      ...
      if (!A.edges.some(ed => ed.from === wiring && ed.to === n.id)) {
        A.edges.push({ from: wiring, to: n.id }); markDirty(); drawWires();
        toast('تم ربط العقدتين — لا تنسَ الحفظ', { type: 'ok', duration: 1800 });
      }
    });
  ...
  function editNode(n) {
    const m = openModal({
      ...
      body: `<div class="field"><label>النوع</label><select class="select" id="en-label">${TYPES[n.type].items.map(i => `<option ${i === n.label ? 'selected' : ''}>${i}</option>`).join('')}</select></div>
        <div class="field"><label>ملاحظة</label><input class="input" id="en-sub" value="${esc(n.sub || '')}"></div>`,
    ...
    m.el.querySelector('#en-save').onclick = () => {
      ...
      markDirty(); m.close(); renderAll();
    };
  }

  function drawWires() {
    wires.querySelectorAll('path:not(.temp)').forEach(p => p.remove());
    wires.setAttribute('width', Math.max(canvas.scrollWidth, 900));
    wires.setAttribute('height', Math.max(canvas.scrollHeight, 560));
    A.edges.forEach(ed => {
      ...
      p.addEventListener('dblclick', () => { A.edges = A.edges.filter(x => x !== ed); markDirty(); drawWires(); });
      wires.appendChild(p);
    });
  }
  function select(id) { selected = id; canvas.querySelectorAll('.bnode').forEach(el => el.classList.toggle('selected', +el.dataset.nid === id)); }
  function renderAll() {
  ...
  function addNode(type, label, x, y) {
    ...
    A.nodes.push(n); markDirty(); renderAll(); select(n.id);
  }
  ...
  document.getElementById('tb-save').onclick = async (e) => {
    const done = btnBusy(e.currentTarget, 'جارٍ الحفظ…');
    try { await DB.automation.save(A); dirty = false; toast('حُفظ سير الأتمتة في قاعدة البيانات', { type: 'ok' }); }
  ...
  document.getElementById('tb-clear').onclick = () => confirmDialog({
    ...
    onConfirm: () => { A = { nodes: [], edges: [], seq: 0 }; markDirty(); renderAll(); }
  });
  document.getElementById('tb-demo').onclick = () => {
    A = {
      ...
      edges: [{ from: 1, to: 2 }, { from: 2, to: 3 }, { from: 2, to: 4 }, { from: 3, to: 5 }, { from: 4, to: 5 }], seq: 5
    };
    markDirty(); renderAll(); toast('حُمّل مثال جاهز — احفظه ليبقى بعد التحديث', { type: 'ok' });
  };
  // معاينة بصرية لمسار التنفيذ — ليست تنفيذًا حقيقيًا
  document.getElementById('tb-run').onclick = () => {
    if (!A.nodes.length) return toast('اللوحة فارغة', { type: 'warn' });
    const order = [];
    const visit = (id) => { if (order.includes(id)) return; order.push(id); A.edges.filter(e => e.from === id).forEach(e => visit(e.to)); };
    A.nodes.filter(n => n.type === 'trigger').forEach(s => visit(s.id));
    A.nodes.forEach(n => !order.includes(n.id) && order.push(n.id));
    toast('معاينة المسار — معاينة بصرية فقط، التنفيذ الفعلي يتطلب Worker', { type: 'info', duration: 2600 });
    order.forEach((nid, i) => setTimeout(() => {
      select(nid);
      if (i === order.length - 1) toast('انتهت المعاينة', { body: `${order.length} عقدة بالترتيب`, type: 'ok' });
    }, i * 650));
  };
  renderAll();
})();
